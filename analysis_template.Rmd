...
author: Diego Mendez Rosa, Yared Osorio, Paola Acevedo & Elizabeth Nolasco.
---
...
length((pharmacoData$cellLine)) 
```

2. What drug concentrations were used in each study?

```{r}
unique(pharmacoData$concentration)
```
...
```{r}
summarizedData <- "summarizedPharmacoData.csv"
summarizePharmaco <- read.csv(summarizedData)
str(summarizePharmaco)
```
...
```{r}
ggplot(pharmacoData, aes(viability, group=study)) + geom_histogram(fill="white", colour="black")
```
